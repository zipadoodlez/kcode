...
This file tracks the current `cargo audit` findings for kcode and the intended remediation path.
It is not an allowlist. It is a triage record so advisories are visible and actionable.

## Current advisories

| Advisory | Crate | Dependency path | Affected area in kcode | Triage | Planned action |
|---|---|---|---|---|---|
...
| `RUSTSEC-2026-0049` | `rustls-webpki` | `rustls` dependency stack (`aws-smithy` rustls 0.21) | TLS certificate revocation list handling | CRLs not considered authoritative by Distribution Point due to faulty matching logic. Transitive via the older rustls stack; fix needs rustls-webpki >=0.103.10, which requires a major bump of the `aws-sdk` stack. | Upgrade rustls/webpki stack when compatible releases are available. |
...
- The `lettre`/`imap` advisories were resolved by cutting the notification and mail integrations; neither crate remains in the graph. `scripts/security_preflight.sh` ignores only the advisories still triaged above (`rustls-webpki`, `lopdf`, `quick-xml`) so CI can remain actionable. New vulnerabilities still fail CI by default.
