...
Kcode has built-in terminal routing for Herdr. When a headed session launch is requested from a client with `HERDR_ENV=1` and `HERDR_PANE_ID`, Kcode splits the calling pane to the right, focuses the new pane, and starts the resumed Kcode session there. `HERDR_BIN_PATH` is honored when present.
...
The sequence must be monotonically increasing for the source. Map Kcode hook sources as follows where possible:
...
```text
kcode --resume <agent_session_id>
```

Kcode session IDs are opaque strings and fit Herdr's ID-based session reference model. No transcript path is needed.
...
1. Add `kcode` to `IntegrationTarget`, CLI parsing, labels, command discovery, recommendations, status, install, and uninstall handling.
2. Install a config-safe Kcode session hook adapter without overwriting an existing user hook. If Herdr cannot safely compose the single Kcode hook command, coordinate a small multi-hook or native-emitter addition in Kcode first.
3. Accept `("herdr:kcode", "kcode")` as an official session source.
4. Persist its ID session reference and map it to `kcode --resume <id>` during restore.
5. Add Kcode process detection and a bundled screen manifest for idle, working, and blocked UI states.
6. Keep screen-manifest detection authoritative until Kcode exposes complete blocked, approval-result, interrupt, and exit transitions.
...
A later Kcode/Herdr protocol can report `working`, `idle`, `blocked`, and `unknown` through `pane.report_agent`, then call `pane.release_agent` on process exit. Do not enable this authority from turn hooks alone. It needs explicit Kcode events for permission/question blocking, approval resolution, cancellation/interrupt, reconnect/reload transfer, and abnormal termination so Herdr never displays a stale working or idle state.
