...
This document describes a proposed evolution of kcode's UI architecture from the
...
Today, kcode is effectively organized like this:
...
- a independent kcode window attached to one session
...
- current independent kcode window
...
2. Client spawns a independent kcode client attached to the same session.
...
This can initially be provided through existing kcode control channels such as:
...
- Preserve current `kcode --resume <session>` workflow.
...
- Should workspace mode be a new command (`kcode workspace`) or a runtime mode of
  the normal client?
...
This gives kcode a portable built-in multi-session workspace without sacrificing
