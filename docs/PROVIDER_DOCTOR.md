...
`kcode provider-doctor` is a user-facing diagnostic that answers one question:
...
tracks (`kcode provider-test-coverage`), but as an interactive command you can run
...
```bash
# Validate kcode's own wiring for a provider, no API key, no spend:
kcode provider-doctor cerebras --tier offline

# Validate the key + live model catalog (needs a key, negligible spend):
kcode provider-doctor cerebras --tier catalog

# Full readiness, including real chat, streaming, and tool calls (spends balance):
kcode provider-doctor cerebras --tier full
...
`kcode provider-test-coverage` shows a cumulative "Recorded spend" footer
...
`kcode provider-test-coverage`. Lighter tiers record the API-dependent
checkpoints as skipped, so they never over-credit a pair.

`kcode provider-test-coverage` renders the same 11 checkpoints as an 11-stage
...
```
  ...
  6/11   nvidia-nim / gemma-4-31b  failed at `streaming reply`; run `kcode provider-doctor nvidia-nim --model gemma-4-31b --tier full`; last tested 2 days ago ...
```
