...
| Try preparation before local context work in both turn paths | Source checks S12 on `crates/jcode-app-core/src/agent/turn_loops.rs` and `crates/jcode-app-core/src/agent/turn_streaming_mpsc.rs` | Passed source checks. Each `prewarm` call precedes `messages_for_provider`; both files compiled in the successful TUI build. |
