# Kcode Server Memory Incident Runbook
...
1. What is using the kcode server's memory?
...
2. Run `kcode debug 'swarm:list'` and inspect the largest live swarm.
...
```bash
kcode debug 'server:memory-incident' > /tmp/before.json
kcode debug 'allocator:purge'
kcode debug 'server:memory-incident' > /tmp/after.json
```
...
1. Run `kcode debug 'server:memory'` for the full attribution walk.
...
kcode debug 'allocator:profile:dump /tmp/jcode-server.heap'
```
