...
| `JCODE_HOOKS_DISABLED` | Always `1`; suppresses hooks in nested kcode calls (recursion guard) |
...
tmux display-message "kcode $icon ${JCODE_HOOK_SESSION_ID:0:12}" 2>/dev/null
notify-send "kcode turn $JCODE_HOOK_STATUS" \
