...
//!
//! This powers `kcode provider-doctor`: it walks the same strict provider/model
...
const FULL_PIPELINE_LABELS: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_JCODE_TOOL_SMOKE, "Real Kcode tool smoke"),
        ...
        checks.push(DoctorCheck::failed(
            ...
            format!(
                "could not select the Claude OAuth credential path: {error}. \
                 Run `kcode login --provider claude` to mint a fresh OAuth token."
            ),
        ));
        return Ok(finish_report(
            provider_id,
...
/// Google OAuth tokens minted by `kcode login --provider antigravity`; the
                ...
                checks.push(DoctorCheck::failed(
                    ...
                    format!(
                        "could not resolve an Antigravity credential: {error}. \
                         Run `kcode login --provider antigravity` to sign in."
                    ),
                ));
                return Ok(finish_report(
                    provider_id,
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            Self::OpenAi => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "kcode login --provider openai",
            },
            Self::Gemini => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "kcode login --provider gemini",
            },
            Self::Cursor => NativeProviderSpec {
                ...
                login_hint: "kcode login --provider cursor",
            },
            Self::Copilot => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "kcode login --provider copilot",
            },
            ...
            Self::Azure => NativeProviderSpec {
                ...
                login_hint: "kcode login --provider azure",
            },
        }
    }
        ...
        match self {
            Self::OpenAi => {
                let credentials = jcode_base::auth::codex::load_credentials()
                    .context("load OpenAI credentials (run `kcode login --provider openai`)")?;
            ...
            Self::Cursor => {
                let key = jcode_base::auth::cursor::load_api_key()
                    .context("load Cursor credential (run `kcode login --provider cursor`)")?;
            ...
            Self::Copilot => {
                let token = jcode_base::auth::copilot::load_github_token()
                    .context("load GitHub Copilot token (run `kcode login --provider copilot`)")?;
                ...
                if !jcode_base::auth::azure::has_configuration() {
                    anyhow::bail!(
                        "Azure OpenAI is not fully configured (need AZURE_OPENAI_ENDPOINT plus an \
                         API key or Entra ID); run `kcode login --provider azure`"
                    );
                }
                Ok(format!(
...
struct NativeProviderSpec {
    ...
    /// `kcode login` hint surfaced when the credential cannot be resolved.
