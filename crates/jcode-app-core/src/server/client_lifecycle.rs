...
fn subscribe_terminal_env(request: &Request) -> Vec<(String, String)> {
    match request {
...
pub(super) async fn handle_client(
    ...
    let mut active_terminal_env = subscribe_terminal_env(&initial_request);
...
async fn start_processing_message(
    ...
    *state.task = Some(tokio::spawn(async move {
        ...
        let result =
            match std::panic::AssertUnwindSafe(process_message_streaming_mpsc_with_terminal_env(
                agent,
                &content,
                images,
                system_reminder,
