...
use jcode_tui_style::theme::{
    ...
    header_name_color, pending_color, queued_color, success_color, tool_color, user_bg, user_color,
    warning_color,
};
...
impl SessionPicker {
    ...
    pub(super) fn highlight_spans(
        ...
        let highlight = base.fg(warning_color()).add_modifier(Modifier::BOLD);
    ...
    pub(super) fn render_session_item_lines_at_frame(
        ...
        let user_clr: Color = user_color();
        let accent: Color = accent_color();
        let batch_restore: Color = error_color();
        ...
        let same_dir_clr: Color = ai_color();
        ...
        line1_spans.extend([
            Span::styled(canary_marker, Style::default().fg(queued_color())),
            Span::styled(debug_marker, Style::default().fg(header_name_color())),
            Span::styled(saved_marker, Style::default().fg(warning_color())),
            Span::styled(
                format!(" {}", status_icon),
        ...
        if let Some(ref label) = session.save_label {
            let label_style = Style::default().fg(warning_color());
    ...
    pub(super) fn render_session_list(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let items: Vec<ListItem> = if let Some(message) = self.loading_message.as_deref() {
            vec![
                ListItem::new(Line::from(vec![
                    ...
                    Span::styled(
                        message.to_string(),
                        Style::default().fg(ai_text()).add_modifier(Modifier::BOLD),
                    ),
                ])),
                ...
                .map(|(idx, item)| {
                    ...
                    match item {
                        ...
                        PickerItem::SavedHeader { session_count } => {
                            let saved_color: Color = warning_color();
                            let line1 = Line::from(vec![
        ...
        if self.loading_message.is_some() {
            ...
            title_parts.push(Span::styled("sessions", Style::default().fg(tool_color())));
        }
