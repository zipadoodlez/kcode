...
\n\
First inspect the codebase, current repo state, and the in-repo docs under `docs/` that describe the current design. Then write a concise ranked todo list using `todo` with the best 3-7 candidate refactors. Prefer behavior-preserving extraction, file splits, dead-code deletion, warning reduction, test isolation, and clearer module boundaries.\n\
\n\
...
\n\
First inspect the codebase, current repo state, and the in-repo docs under `docs/` that describe the current design. Then write a concise ranked todo list using `todo` with the best 3-7 refactors to tackle next. Prefer behavior-preserving extraction, splitting oversized modules, dead-code deletion, warning reduction, test improvements, and boundary clarification.{}\n\
\n\
