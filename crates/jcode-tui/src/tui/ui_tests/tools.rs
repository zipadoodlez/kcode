use super::*;
use jcode_tui_style::theme::{error_color, success_color, tool_color, warning_color};
...
fn test_tool_activity_detail_hides_technical_summary_by_default() {
    let call = ToolCall {
        id: "call_intent_only".to_string(),
        name: "schedule".to_string(),
        input: serde_json::json!({
            "action": "list",
            "intent": "Check the CI schedule"
        }),
        intent: Some("Check the CI schedule".to_string()),
        thought_signature: None,
    };
    let detail = tools_ui::get_tool_activity_detail(&call);
    assert_eq!(detail, "Check the CI schedule");
}
...
fn test_render_batch_subcall_line_keeps_full_bash_summary_when_row_fits() {
    ...
    let line =
        tools_ui::render_batch_subcall_line(&tool, "✓", success_color(), 32, Some(160), None);
...
fn test_render_batch_subcall_line_shows_model_provided_intent() {
    ...
    let line =
        tools_ui::render_batch_subcall_line(&tool, "✓", success_color(), 50, Some(120), None);
    let rendered = extract_line_text(&line);
