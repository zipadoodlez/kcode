...
use crate::openai_stream_runtime::{effective_ws_completion_timeout_secs, request_reasoning_effort};

#[test]
fn ws_completion_timeout_scales_with_request_reasoning_effort() {
    let base = effective_ws_completion_timeout_secs(&serde_json::json!({"model": "gpt-5.6"}));
    assert!(base > 0);

    // A request without reasoning keeps the base budget.
    assert_eq!(
    ...
    // websocket completion budget must grow with it instead of killing the
    // stream mid-thought.
...
#[test]
fn request_reasoning_effort_reads_the_responses_payload_shape() {
    assert_eq!(
